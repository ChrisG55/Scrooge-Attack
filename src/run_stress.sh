...
case $1 in
	...
	STRESSORS="--exclude $($STRESSNGEXEC --class filesystem? | sed -e 's/^.\+: \(.\+\)$/\1/' -e 's/[[:space:]]\+/,/g'),af-alg,affinity,apparmor,bad-ioctl,bind-mount,cyclic,dccp,ioport,ipsec_mb,memhotplug,mlock,mlockmany,netlink-proc,numa,oom-pipe,pidfd,pkey,rdrand,sctp,sysinfo,tree,tsc,userfaultfd,watchdog --matrix-size 4096 --matrix-3d-size 256 --memrate-bytes 128m --sequential 0 --tsearch-size 1048576 --vm-rw-bytes 8m"
	;;
    energy)
	# crypt: CPU-bound stressor
	# malloc: memory-bound stressor
	STRESSORS="--crypt 0 --malloc 0 --timeout 60"
	;;
    -h|--help)
	usage
	exit 0
...
sudo $STRESSNGEXEC --abort --aggressive $STRESSORS --log-file ./stress-ng.log --maximize --metrics-brief --times --timestamp --verify
